...
var designation_typeModel = require('./../models/designation_typeModel');


router.post('/create', async function(req, res) {
  try{
 let doctor_specModels  =  await designation_typeModel.findOne({designation_type:req.body.designation_type});

  if(doctor_specModels == null){
    await designation_typeModel.create({
...
router.post('/filter_date', function (req, res) {
        designation_typeModel.find({}, function (err, StateList) {
          var final_Date = [];
...
router.get('/deletes', function (req, res) {
      designation_typeModel.remove({}, function (err, user) {
...
router.post('/getlist_id', function (req, res) {
        designation_typeModel.find({Person_id:req.body.Person_id}, function (err, StateList) {
...
router.get('/getlist', function (req, res) {
        designation_typeModel.find({}, function (err, Functiondetails) {
...
router.post('/edit', function (req, res) {
        designation_typeModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
...
router.post('/delete', function (req, res) {
      designation_typeModel.findByIdAndRemove(req.body._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
