...
router.post('/create', async function(req, res) {
  try{
 let doctor_specModels  =  await usergroupModel.findOne({usergroup:req.body.usergroup});
...
router.post('/filter_date', function (req, res) {
        usergroupModel.find({}, function (err, StateList) {
          var final_Date = [];
...
router.get('/deletes', function (req, res) {
      usergroupModel.remove({}, function (err, user) {
...
router.post('/getlist_id', function (req, res) {
        usergroupModel.find({Person_id:req.body.Person_id}, function (err, StateList) {
...
router.get('/getlist', function (req, res) {
        usergroupModel.find({}, function (err, Functiondetails) {
...
router.post('/edit', function (req, res) {
        usergroupModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
...
router.post('/delete', function (req, res) {
      usergroupModel.findByIdAndRemove(req.body._id, function (err, user) {
          if (err) return res.json({Status:"Failed",Message:"Internal Server Error", Data : {},Code:500});
